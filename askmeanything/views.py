...
def vote(request, poll_id):
    ...
    return HttpResponseRedirect(
        reverse('askmeanything.views.results', kwargs={'poll_id': poll_id})
    )
...
def results(request, poll_id):
    ...
    return render_to_response(
        'askmeanything/poll_results.html',
        {'poll': poll, 'poll_results': poll_results},
        mimetype='text/plain'
    )
...
def created(request, poll_id):
    poll = get_object_or_404(Poll, id=poll_id)
    return render_to_response('askmeanything/poll_post_create.html', {'poll': poll})